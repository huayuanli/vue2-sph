        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in saleAttrList" :key="item.id">
                <dt class="title">{{ item.saleAttrName }}</dt>
                <dd changepirce="0" :class="{ active: saleValue.isChecked == '1' }"
                  v-for="(saleValue, index) in item.spuSaleAttrValueList"
                  @click="changeChecked(saleValue, item.spuSaleAttrValueList)">{{ saleValue.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
export default {
  ...
  methods: {
    changeChecked(saleValue, spuSaleAttrValueList) {
      console.log(saleValue, spuSaleAttrValueList);
    ...
    async addShop() {
      try {
        await this.$store.dispatch('getAddOrUpdateShopcart', { skuId: this.$route.params.skuid, skuNum: this.num })
        //路由跳转
