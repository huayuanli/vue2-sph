...
export default {
  ...
  methods: {
    async getOrderInfo() {
      const result = await this.$TradeAPI.reqOrderInfoApi(this.orderId)
      this.orderInfo = result.data.data
    },
    async open() {
      const result = await QRCode.toDataURL(this.orderInfo.codeUrl)

      this.$alert(`<img src="${result}" />`, 'HTML 片段', {
      ...
      if (!this.timeId) {
        this.timeId = setInterval(async () => {
          const result = await this.$TradeAPI.reqPayStatusApi(this.orderId)
